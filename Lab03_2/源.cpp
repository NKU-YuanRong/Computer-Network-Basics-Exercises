...
#define error_rate 5    // 发送端错误率
#define lost_rate 0     // 发送端丢包率
...
int lptr = 0, rptr = winLen, senptr = 0; // lptr、rptr滑动窗口指针  senptr当前发送包指针
std::mutex mtx;         // 小锁一手，防止出现并行问题


int maxPackge = 0;      // 发送文件的最大包数量（会在程序运行时被初始化）
int maxTime = 200;      // 最大发送间隔
        ...
        if (senptr < rptr && senptr < maxPackge) {
            ...
            if (senptr == maxPackge) {
                ...
                if (not_to_last == 1) {
                    ...
                    cout << "Change senptr to " << lptr << endl;
                    senptr = lptr;
                    not_to_last = -1;
                    resend_last++;
                }
            ...
            cout << "time out" << endl;
            cout << "lptr " << lptr << ", rptr " << rptr << ", senptr " << senptr << endl;
            mtx.unlock();
            Sleep(100);
        }
    }
